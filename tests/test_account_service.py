...
def test_account_and_transaction_crud():
    # Create account
    response = client.post("/accounts/", json={"name": "TestAccount", "initial_balance": 0, "transactions": []})
    assert response.status_code == 200

    # Read account
    response = client.get("/accounts/TestAccount")
    assert response.status_code == 200
    assert response.json() == {"name": "TestAccount", "initial_balance": 0, "transactions": []}

    # Update account
    response = client.put("/accounts/TestAccount", json={"name": "TestAccount", "initial_balance": 50, "transactions": []})
    assert response.status_code == 200

    # Verify update
    response = client.get("/accounts/TestAccount")
    assert response.status_code == 200
    assert response.json() == {"name": "TestAccount", "initial_balance": 50, "transactions": []}

    # Add transaction
    transaction_data = {"transaction_date": datetime.now().isoformat(), "description": "TestTransaction", "debit": 20}
    ...
    # Update transaction
    transaction_data = {"transaction_date": datetime.now().isoformat(), "description": "UpdatedTransaction", "debit": 30}
